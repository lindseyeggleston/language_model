...
class DataLoader(object):
    """
    Handles all aspects of the data. Stores the dataset_params, vocabulary and
    tags with their mappings to indices.
    """
    ...
    def __init__(self, data_dir, params):
        ...
        if not os.path.isfile(json_path):
            raise ValueError("No json file found at {}, run build_vocab.py".format(json_path))
    ...
    def load_sentences_labels(self, sentences_file, labels_file, d):
        """
        Loads sentences and labels from their corresponding files. Maps tokens
        and tags to their indices and stores them in the provided dict d.
    ...
    def load_data(self, types, data_dir):
        ...
        for split in ['train', 'val', 'test']:
            if split in types:
                ...
                data[split] = {}
                self.load_sentences_labels(sentences_file,
                                           labels_file,
                                           data[split])
    ...
    def data_iterator(self, data, params, shuffle=False):
        """
        Returns a generator that yields batches data with labels. Batch size
        is params.batch_size. Expires after one pass over the data.
        ...
        Yields
        ------
        batch_data: (Variable) dimension batch_size x seq_len with the sentence
            data
        batch_labels: (Variable) dimension batch_size x seq_len with the
            corresponding labels
        """
        # make a list that decides the order in which we go over the data
        # this avoids explicit shuffling of data
